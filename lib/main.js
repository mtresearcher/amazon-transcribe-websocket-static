...
var AWSPoolID = 'us-west-2:bc50d139-f2d1-41c5-af38-d2655785c9da';
...
function setLanguage() {
    // setting sample rate for some languages to 44khz and some to 16khz. Why?
...
function disableLanguage() {
    // retrieves the language for transcription from streaming.html and disables it?
  var currentLanguage;
...
function formatTranslate(){
  ...
  if (currentLanguage == 'he' || currentLanguage == 'ar' || currentLanguage == 'ur') {
...
let handleEventStreamMessage = function (messageJson) {
    ...
    if (results.length > 0) {
        if (results[0].Alternatives.length > 0) {
            ...
            // update the textarea with the latest translation result
            translateInput(transcript, function(translated){
                console.log("translated: " + translated);
                var words = translated.split(/\W+/);
                // similar to mask k
                for (let i = 0; i < mask_k; i++) {
                    words.pop();
                }
                $('#translate').val(translation + words.join(' ') + "\n");
...
function translateInput(text, callback) {
  // var source_language = document.getElementById('language');
  var source_language = $('#language').find(':selected').val();
